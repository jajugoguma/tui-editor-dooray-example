...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		root["toastui"] = root["toastui"] || {}, root["toastui"]["Editor"] = root["toastui"]["Editor"] || {}, root["toastui"]["Editor"]["plugin"] = root["toastui"]["Editor"]["plugin"] || {}, root["toastui"]["Editor"]["plugin"]["autoComplete"] = factory();
})(self, function() {
...
__webpack_require__.d(__webpack_exports__, {
  "default": function() { return /* binding */ autoCompletePlugin; }
});
...
var pluginKey = new PluginKey('tui-editor-plugin-auto-complete');
function isAutoCompleteMode(selection, decorations) {
...
function closeAutoComplete(view) {
    var plugin = pluginKey.get(view.state);
    var meta = { act: TR_ACTIONS.REMOVE };
    var tr = view.state.tr.setMeta(plugin, meta);
    view.dispatch(tr);
    return true;
}

;// CONCATENATED MODULE: ./src/plugins/plugin.ts
...
function autoComplete(context, reducer) {
    var deactivateAutoCompleteState = {
        active: false,
        decorations: context.pmView.DecorationSet.empty,
    };
    var plugin = new context.pmState.Plugin({
        key: pluginKey,
        view: function () {
            return {
                update: function (view, prevState) {
                    var _a, _b, _c, _d;
                    var prev = pluginKey.getState(prevState);
                    var next = pluginKey.getState(view.state);
                    var started = !prev.active && next.active;
                    var stopped = prev.active && !next.active;
                    var changed = next.active && !started && !stopped && prev.filter !== next.filter;
                    var action = {
                        view: view,
                        ...
                        triggerType: (_d = next.triggerType) !== null && _d !== void 0 ? _d : prev.triggerType,
                    };
                    if (started) {
                        reducer(__assign(__assign({}, action), { act: USER_ACTIONS.OPEN }));
                    }
                    if (changed) {
                        reducer(__assign(__assign({}, action), { act: USER_ACTIONS.FILTER }));
                    }
                    if (stopped) {
                        reducer(__assign(__assign({}, action), { act: USER_ACTIONS.CLOSE }));
                    }
                },
            };
        },
        state: {
            ...
            apply: function (tr, state) {
                ...
                if ((meta === null || meta === void 0 ? void 0 : meta.act) === TR_ACTIONS.ADD) {
                    var trigger_1 = meta.trigger, filter = meta.filter, triggerType_1 = meta.triggerType;
                    var from_1 = tr.selection.from - trigger_1.length - ((_a = filter === null || filter === void 0 ? void 0 : filter.length) !== null && _a !== void 0 ? _a : 0);
                    var to_1 = tr.selection.from;
                    var attrs = __assign(__assign({}, triggerType_1 === null || triggerType_1 === void 0 ? void 0 : triggerType_1.decorationAttrs), { class: 'dooray-auto-complete' });
                    var deco = context.pmView.Decoration.inline(from_1, to_1, attrs, {
                ...
                if ((meta === null || meta === void 0 ? void 0 : meta.act) === TR_ACTIONS.REMOVE ||
                    !isAutoCompleteMode(tr.selection, nextDecorations) ||
                    !hasDecoration) {
                    return deactivateAutoCompleteState;
                }
                var active = state.active, trigger = state.trigger, triggerType = state.triggerType;
                var nextDecoration = nextDecorations.find()[0];
                var from = nextDecoration.from, to = nextDecoration.to;
        ...
        props: {
            ...
            handleKeyDown: function (view, ev) {
                var _a, _b;
                var _c = this.getState(view.state), trigger = _c.trigger, active = _c.active, decorations = _c.decorations, triggerType = _c.triggerType;
                if (!active || !isAutoCompleteMode(view.state.selection, decorations)) {
                    return false;
                }
                var deco = decorations.find()[0];
                var from = deco.from, to = deco.to;
                var text = view.state.doc.textBetween(from, to);
                var filter = text.slice((_a = trigger === null || trigger === void 0 ? void 0 : trigger.length) !== null && _a !== void 0 ? _a : 1);
                var isCancelWithSpace = (_b = triggerType === null || triggerType === void 0 ? void 0 : triggerType.cancelWithSpace) !== null && _b !== void 0 ? _b : true;
                ...
                if (filter.length === 0 && ev.key === 'Backspace') {
                    context.pmRules.undoInputRule(view.state, view.dispatch);
                    closeAutoComplete(view);
                    return true;
                }
                var userAction = getActions(ev);
                var currentAction = {
                    view: view,
                    trigger: trigger,
                    filter: filter,
                ...
                switch (userAction) {
                    ...
                    case USER_ACTIONS.ENTER:
                        return (reducer(__assign(__assign({}, currentAction), { act: USER_ACTIONS.ENTER })) || closeAutoComplete(view));
                    case USER_ACTIONS.UP:
                    case USER_ACTIONS.DOWN:
                    case USER_ACTIONS.LEFT:
                    case USER_ACTIONS.RIGHT:
                        return reducer(__assign(__assign({}, currentAction), { act: userAction }));
                    default:
                        break;
                }
                return false;
...
;// CONCATENATED MODULE: ./src/plugins/trigger.ts


var reBlockClass = /(code-block|custom-block)/;
var reBlankChracters = /^\s*$/;
function isTriggeringAutoComplete(state, context) {
    var selection = state.tr.selection;
    var _a = selection.$from, depth = _a.depth, parentOffset = _a.parentOffset;
    var node = selection.$from.node(depth);
    var prevCharacter = node.textContent.charAt(parentOffset - 1);
    var plugin = pluginKey.get(state);
    var decorationSet = new context.pmView.DecorationSet();
    return (reBlankChracters.test(prevCharacter) &&
        !reBlockClass.test(node.attrs.className) &&
        !plugin.getState(state).active &&
        !isAutoCompleteMode(state.selection, decorationSet));
}
function autoCompleteTrigger(context, triggers) {
    var rules = triggers.map(function (trigger) {
        return new context.pmRules.InputRule(trigger.regex, function (state, match) {
            if (isTriggeringAutoComplete(state, context)) {
                var tr = state.tr.insertText(match[1][match[1].length - 1]);
                var meta = { act: TR_ACTIONS.ADD, trigger: match[1], triggerType: trigger };
    ...
    return context.pmRules.inputRules({ rules: rules });
}
    ...
    return function (action) {
        ...
        switch (action.act) {
            ...
            case USER_ACTIONS.FILTER:
                return (_f = (_e = options.onFilter) === null || _e === void 0 ? void 0 : _e.call(options, action)) !== null && _f !== void 0 ? _f : false;
            case USER_ACTIONS.ENTER:
                return (_h = (_g = options.onEnter) === null || _g === void 0 ? void 0 : _g.call(options, action)) !== null && _h !== void 0 ? _h : false;
            case USER_ACTIONS.UP:
            case USER_ACTIONS.DOWN:
            case USER_ACTIONS.LEFT:
            case USER_ACTIONS.RIGHT:
                return (_k = (_j = options.onArrow) === null || _j === void 0 ? void 0 : _j.call(options, action)) !== null && _k !== void 0 ? _k : false;
            default:
                return false;
        }
    };
}
function autoCompletePlugin(context, options) {
    if (options === void 0) { options = {}; }
    var opts = src_assign({ triggers: [], reducer: defaultReducer(options) }, options);
    var reducer = opts.reducer, triggers = opts.triggers;
    return {
        markdownPlugins: [
            function () { return autoComplete(context, reducer); },
            function () { return autoCompleteTrigger(context, triggers); },
        ],
        wysiwygPlugins: [
            function () { return autoComplete(context, reducer); },
            function () { return autoCompleteTrigger(context, triggers); },
